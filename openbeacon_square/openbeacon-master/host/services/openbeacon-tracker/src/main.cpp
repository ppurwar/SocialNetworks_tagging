...
static const long tea_keys[][XXTEA_BLOCK_COUNT] = {
    ...
    {0x9c43725e, 0xad8ec2ab, 0x6ebad8db, 0xf29c3638},      /* 6: The Last Hope - AMD Project  */
    {0xf6e103d4, 0x77a739f6, 0x65eecead, 0xa40543a9},      /* 7: strange key from 25C3 */
    {0x0767d85e, 0x23cfb28e, 0x101faeba, 0x069a2665},      /* new Keys from Martin */
              ...
              bool decrypt)
{
    ...
    {
#ifdef PPDBG
        printf("enterlen=%d,%d,%d\t",len,sizeof (TBeaconLogSighting),
                        (len % sizeof (TBeaconLogSighting)));//PPDBG
#endif
...
#ifdef PPDBG
                printf("readerId=%d\t",reader_id);//PPDBG
#endif
                data = &log->log;
                len = sizeof (log->log);
#ifdef PPDBG
                printf("len=%d\t",len);//PPDBG
#endif
...
#ifdef PPDBG
            printf("readerId=%x\t",reader_id);//PPDBG
#endif
            timestamp = microtime ();
#ifdef READER_UPDATE
            if(size > 32)
